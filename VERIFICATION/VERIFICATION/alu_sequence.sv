...
class err_sequence extends alu_sequence;
        ...
        virtual task body();
                ...
                // 1. Send initial transaction with inp_valid = 2'b01 (Op A only)
                trans = alu_transaction::type_id::create("trans");
                start_item(trans);
                `uvm_info("err_sequence", "Starting timeout sequence: sending Op A", UVM_LOW)
                assert(trans.randomize() with {
                        ce        == 1'b1;
                        inp_valid == 2'b01;
                        mode      == 1'b1;
                ...
                // Save values so the waiting cycles maintain the same command/opa context if needed
                saved_opa = trans.opa;
                saved_cmd = trans.cmd;
                
//                trans.print();
                `uvm_info("err_sequence", "Done generating initial transaction", UVM_LOW)
                finish_item(trans);

                // 2. Repeat 17 cycles withholding Op B to trigger the 16-cycle timeout error
                repeat(17) begin
                        trans = alu_transaction::type_id::create("trans");
                        start_item(trans);
                        `uvm_info("err_sequence", "Generating timeout wait cycle", UVM_LOW);
                        
                        assert(trans.randomize() with {
                                ce        == 1'b1;
                                inp_valid == 2'b01; // Withholding Op B
                                mode      == 1'b1;
                                cmd       == saved_cmd;
                                opa       == saved_opa;
                        });
                        
                        `uvm_info("err_sequence", "Done generating wait cycle", UVM_LOW);
                        finish_item(trans);
                end
        endtask
endclass*/
